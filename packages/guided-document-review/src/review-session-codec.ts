...
export async function decodeReviewSession(
  ...
  const answers = decodeAnswers(input.answers, checkpointSet, errors);
  const events = decodeEvents(input.events, checkpointSet, errors);

  if (answers && checkpointIndex !== null && stepIndex !== null && phase) {
    checkSessionPosition(checkpointSet, answers, checkpointIndex, stepIndex, phase, errors);
  }
  ...
  if (answers && events && checkpointIndex !== null && typeof input.startedAt === "string") {
    checkEventConsistency(checkpointSet, answers, events, checkpointIndex, input.startedAt, errors);
  }
  if (errors.length || !reviewer || !phase || checkpointIndex === null || stepIndex === null ||
      revision === null || !answers || !events || typeof input.sessionId !== "string" ||
      typeof input.startedAt !== "string" || typeof input.updatedAt !== "string") {
...
function decodeReviewer(input: unknown, errors: string[]): ReviewerIdentity | null {
  if (!isRecord(input)) {
    errors.push("Reviewer must be a plain object.");
    return null;
  }
  ...
  if (input.kind === "self_asserted" && validTrimmedString(input.name, 1)) {
    return { kind: "self_asserted", name: input.name };
  }
...
function decodeAnswers(
  ...
  checkpointSet: CheckpointSet,
  errors: string[],
): Readonly<Record<string, CheckpointAnswer>> | null {
  if (!isRecord(input)) {
    errors.push("Review answers must be a plain object.");
    return null;
  }
  ...
  if (Object.keys(input).sort().join("|") !== expectedKeys.join("|")) {
    errors.push("Review answer keys must exactly match the checkpoint set.");
  }
  const decoded: Record<string, CheckpointAnswer> = {};
  checkpointSet.checkpoints.forEach((checkpoint) => {
    const answer = decodeAnswer(input[checkpoint.checkpointId], checkpoint, errors);
    if (answer) decoded[checkpoint.checkpointId] = answer;
  });
  return Object.keys(decoded).length === checkpointSet.checkpoints.length ? decoded : null;
}

function decodeAnswer(input: unknown, checkpoint: ReviewCheckpoint, errors: string[]): CheckpointAnswer | null {
  ...
  checkExactKeys(input, answerFields, label, errors);
  ...
  const revealed = Array.isArray(input.revealedStepIds) && input.revealedStepIds.every((value) => typeof value === "string")
    ? [...input.revealedStepIds] as string[] : null;
  if (!revealed) {
    errors.push(`${label} revealed steps must be a string array.`);
  } else if (revealed.some((stepId, index) => stepId !== stepIds[index])) {
    errors.push(`${label} revealed steps must be an ordered prefix of its learning steps.`);
  }
  ...
  const confidence = input.confidence === null ? null : integerBetween(input.confidence, 1, 5)
    ? input.confidence as 1 | 2 | 3 | 4 | 5 : undefined;
  ...
  if (errors.length !== startErrors || !revealed || explanation === undefined || confidence === undefined ||
      decision === undefined || decidedAt === undefined) return null;
  return {
...
function decodeReplacement(
  ...
  if (!isRecord(input)) {
    errors.push(`${label} replacement must be a plain object or null.`);
    return null;
  }
  const startErrors = errors.length;
  checkExactKeys(input, replacementFields, `${label} replacement`, errors);
  const step = typeof input.stepId === "string"
    ? checkpoint.learningSteps.find(({ stepId }) => stepId === input.stepId) : undefined;
  ...
  if (errors.length !== startErrors || !step || typeof input.original !== "string" ||
      typeof input.replacement !== "string" || !(input.rationale === null || typeof input.rationale === "string")) return null;
...
function decodeEvents(input: unknown, checkpointSet: CheckpointSet, errors: string[]): readonly ReviewEvent[] | null {
  if (!Array.isArray(input)) {
    errors.push("Review events must be an array.");
    return null;
  }
  ...
  input.forEach((entry, index) => {
    ...
    const checkpoint = typeof entry.checkpointId === "string"
      ? checkpointSet.checkpoints.find(({ checkpointId }) => checkpointId === entry.checkpointId) : undefined;
    if (!checkpoint) errors.push(`${label} checkpoint ID is invalid.`);
    if (!(entry.stepId === null || (typeof entry.stepId === "string" && checkpoint?.learningSteps.some(({ stepId }) => stepId === entry.stepId)))) {
      errors.push(`${label} step ID is invalid.`);
    }
    ...
    if (errors.length !== startErrors || typeof entry.eventId !== "string" || !checkpoint ||
        !(entry.stepId === null || typeof entry.stepId === "string") || !phases.has(entry.phase as ReviewPhase) ||
        typeof entry.revision !== "number" || typeof entry.recordedAt !== "string") return;
    decoded.push({
      eventId: entry.eventId,
      checkpointId: checkpoint.checkpointId,
      stepId: entry.stepId,
      phase: entry.phase as ReviewPhase,
...
function checkSessionPosition(
  ...
  if (!checkpoint) {
    errors.push("Current checkpoint index is outside the checkpoint set.");
    return;
  }
  if (!checkpoint.learningSteps[stepIndex]) errors.push("Current step index is outside the active checkpoint.");
  checkpointSet.checkpoints.forEach((candidate, index) => {
    ...
    if (index === checkpointIndex && phase !== "complete" && answer.decision) errors.push("The active checkpoint cannot already be decided.");
  ...
  if (phase === "complete") {
    if (checkpointIndex !== checkpointSet.checkpoints.length - 1 || stepIndex !== 0 ||
        checkpointSet.checkpoints.some(({ checkpointId }) => !answers[checkpointId].decision)) {
      errors.push("A complete review must finish every checkpoint and reset the step index.");
    }
    return;
  }
  if (phase === "orient" && stepIndex !== 0) errors.push("Orient phase must use the first learning-step index.");
  if (phase === "learn" && answer.revealedStepIds.length < stepIndex) {
    errors.push("Learn phase cannot skip unrevealed learning steps.");
  }
  if (["explain_back", "confidence", "decide"].includes(phase) &&
...
function checkEventConsistency(
  ...
  events.forEach((event, index) => {
    ...
    if (eventCheckpointIndex < previousCheckpointIndex || eventCheckpointIndex > checkpointIndex) {
      errors.push(`Review event ${index + 1} is outside the checkpoint progression.`);
    }
  ...
  if (events.length && (events[0].checkpointId !== checkpointSet.checkpoints[0].checkpointId || events[0].phase !== "orient")) {
    errors.push("The first review event must orient the first checkpoint.");
  }

  checkpointSet.checkpoints.forEach((checkpoint) => {
    const answer = answers[checkpoint.checkpointId];
    const checkpointEvents = events.filter((event) => event.checkpointId === checkpoint.checkpointId);
    answer.revealedStepIds.forEach((stepId) => {
      if (!checkpointEvents.some((event) => event.phase === "learn" && event.stepId === stepId)) {
        errors.push(`Revealed step ${stepId} has no matching learning event.`);
      }
    });
    if (answer.explanation !== null && !checkpointEvents.some((event) => event.phase === "explain_back")) {
      errors.push(`Checkpoint ${checkpoint.checkpointId} explanation has no matching event.`);
    }
...
function isPristine(answer: CheckpointAnswer): boolean {
  return answer.revealedStepIds.length === 0 && answer.explanation === null && answer.confidence === null &&
    answer.decision === null && answer.replacement === null && answer.decidedAt === null;
}

function checkIdentity(actual: unknown, expected: string, label: string, errors: string[]): void {
  if (actual !== expected) errors.push(`Review session ${label} does not match.`);
}

function checkNonEmptyString(value: unknown, label: string, errors: string[]): void {
  if (!validTrimmedString(value, 1)) errors.push(`${label} is invalid.`);
}

function checkTimestamp(value: unknown, label: string, errors: string[]): void {
  if (!validTimestamp(value)) errors.push(`${label} is invalid.`);
}

function checkExactKeys(value: Record<string, unknown>, fields: readonly string[], label: string, errors: string[]): void {
  if (Object.keys(value).sort().join("|") !== [...fields].sort().join("|")) errors.push(`${label} has an invalid shape.`);
}

function validTrimmedString(value: unknown, minimum: number): value is string {
