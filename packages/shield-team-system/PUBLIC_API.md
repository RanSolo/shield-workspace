...
| `@shield/team-system/guided-review` | Content-addressed `guided.review.v1` plan and human-review engine with Product QA, code, and document/spike playbooks; several one-question steps form durable stages, corrections stale only dependent steps, and the non-authoritative exact-candidate Yes/No/Cancel fork can be bound by the prepared-publication host into one signed key turn without granting the evaluator signing, journal, or publication effects |
