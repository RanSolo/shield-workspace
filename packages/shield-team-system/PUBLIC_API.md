...
| `@shield/team-system/guided-review` | Content-addressed `guided.review.v1` human-review engine with Product QA, code, and document/spike playbooks; several one-question steps form durable stages, corrections stale only dependent steps, and the non-authoritative publication fork yields exact-candidate Yes/No/Cancel PIN intent without signing, journal mutation, or publication effects |
