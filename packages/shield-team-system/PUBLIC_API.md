...
| `@shield/team-system/feature-operation` | Pure host-neutral, independent `feature.operation.v1` and hardened `feature.operation.v2` plan, authority, replay-context, derived-candidate, digest, signature-verification, and bounded eligibility contracts; V1 and V2 reject one another and perform no signing, journal append, dispatch, publication, integration, rollback, merge, deployment, or release effect |
| `@shield/team-system/tdd-mission` | Pure host-neutral, non-authoritative `tdd.mission.v1` evaluator; it performs no signing, journal append, dispatch, test execution, publication, or human-decision effect |
| `@shield/team-system/feature-integration` | Canonical legacy V1 feature-operation journal/store plus additive hardened V2 journal primitives, immutable producer/human trust roots, and a secure dispatcher that blocks V1 operational replay as `LEGACY_JOURNAL_UNTRUSTED`; existing V1 replay remains available only for compatibility evidence |
...
| `@shield/team-system/mission-builder` | Non-authoritative deterministic mission definitions for five bounded patterns, exact provenance/proofreading replay, one-edge advance, Mack host receipts, human wait gates, and replay-derived status |
...
| `@shield/team-system/guided-review` | Content-addressed `guided.review.v1` plan and human-review engine with Backend, Frontend, and Spike playbooks plus cross-cutting code review; several one-question steps form durable stages, named participants and versioned driver receipts preserve identity boundaries, corrections stale only dependent steps, and the non-authoritative exact-candidate Yes/No/Cancel fork can be bound by the prepared-publication host into one signed key turn without granting the evaluator signing, journal, or publication effects. The `guided.review.projection.v1` surface adds an ephemeral, authority-none, exact-session-step projection of bounded local literal diff anchors. Content-addressed `guided.review.question.v1` and `guided.review.answer.v1` envelopes require the operator to replay the displayed exact question digest through a closed ASCII answer grammar before one existing session CAS; incomplete answers are zero-mutation and complete answers use a projection-file-independent read-only context seam. The host securely reads a deterministic, exact-bound `guided.review.automated-check-source.v1` and projects authority-none `guided.review.automated-check.v1` receipts with exact source-byte hashes; question callers cannot inject check evidence. Pure `guided.review.follow-up.v1` records exact findings and issue identity with no external-effect authority. |
