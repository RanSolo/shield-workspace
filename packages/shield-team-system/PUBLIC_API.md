...
| `@shield/team-system/feature-operation` | Pure host-neutral `feature.operation.v1` plan, signed Epic Wheels Up verification, trusted replay validation, amendment comparison, and bounded derived-candidate eligibility; it performs no signing, journal append, dispatch, publication, integration, rollback, merge, deployment, or release effect |
| `@shield/team-system/feature-integration` | Canonical feature-operation journal/store, exact child and rollback evidence bridges, bounded draft workspace and feature-branch integration, separately authorized cumulative validation, and a replay-first one-stage controller; feature-to-main merge, ready state, deployment, and release are unrepresentable |
