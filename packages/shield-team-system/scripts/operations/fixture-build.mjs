...
import { hashFile, sha256, stableJson } from './common.mjs';
...
const makePdf = () => {
  ...
  const content2 = [
    ...
    'BT /F1 11 Tf 36 690 Td (Variable overlay area:) Tj ET',
    '0.8 w 36 500 540 170 re S',
    'BT /F1 11 Tf 36 460 Td (Stamp area:) Tj ET',
    '0.8 w 36 330 220 110 re S',
    'BT /F1 11 Tf 300 460 Td (Signature image area:) Tj ET',
    '0.8 w 300 330 276 110 re S',
...
const fixtureData = () => ({
  ...
  customer: {
    name: 'Northwind Demonstration Cooperative',
    accountId: 'SYNTH-00449',
    contactName: 'Alex Example',
    address: ['100 Example Avenue', 'Testville, KS 66002'],
  },
  report: {
    title: 'Annual Service and Compliance Summary',
  ...
  modification: {
    overlayText: 'Account SYNTH-00449 — reviewed 2026-08-08',
    stampLabel: 'SYNTHETIC APPROVED',
    signerLabel: 'Example Signer — test fixture only',
    publicTestPassword: 'nxt449-test-only',
  },
});

export const buildFixture = async ({ outputDirectory }) => {
  const output = resolve(outputDirectory);
  if (existsSync(output)) throw new Error(`Refusing existing fixture directory: ${output}`);
  ...
  const sourcePdfPath = join(output, 'source-template.pdf');
  ...
  const encryptedPath = join(output, 'encrypted-input.pdf');
  execFileSync('gs', [
  ...
  await write('expected-observations.json', stableJson({
    ...
    fullDocument: {
      minimumPages: 2,
      requiredLineItemIds: ['EX-001', 'EX-017', 'EX-072'],
      requireHeaderFooter: true,
      requireStablePageNumbers: true,
      requireReopen: true,
    },
    modification: {
      sourcePageCount: 2,
      sourceMediaBoxPoints: [0, 0, 612, 792],
      requireOverlayText: true,
      requireStampImage: true,
      requireSignatureImage: true,
      requirePreservedPageDimensions: true,
      malformedInputMustNotSilentlySucceed: true,
      encryptedInputMustBeRecorded: true,
    },
  }));
  await write('README.md', `# ${FIXTURE_ID}\n\nThis fixture contains synthetic test data and programmatically generated assets only. It contains no customer data, company branding, copied fonts, private keys, credentials, or human signatures. The password for the encrypted negative-test PDF is public fixture data, not a secret.\n\nUse the manifest hash supplied by Feature Hill. Do not substitute repository test PDFs or mutable external files.\n`);
  ...
  const manifest = {
    ...
    assetProvenance: 'programmatically-generated-for-this-local-test-fixture',
  ...
  const receipt = {
    ...
    tool: { path: toolPath, version: TOOL_VERSION, sha256: sha256(await readFile(toolPath)) },
    ghostscriptVersion: execFileSync('gs', ['--version'], { encoding: 'utf8' }).trim(),
  };
  await writeFile(join(output, 'build-receipt.json'), stableJson(receipt), { flag: 'wx' });
  return { output, manifest, manifestHash };
};

const main = async () => {
  const argv = process.argv.slice(2);
  if (argv.shift() !== '--output' || argv.length !== 1) throw new Error('Usage: fixture-build.mjs --output NEW_DIRECTORY');
