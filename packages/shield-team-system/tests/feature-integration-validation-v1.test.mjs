...
function executionFixture(overrides = {}) {
  ...
  const request = { schemaVersion: 1, operationId: "operation:test", repositoryId: "RanSolo/shield-workspace", terminalHeadRevision: "a".repeat(40), terminalTreeDigest: digest("a"), commands: [command], commandIds: [command.commandId], targetIds: [...command.targetIds], validationIds: ["test"], requestDigest: digest("0") };
  ...
  return { command, request, signedAuthority, trustedBindings: [binding], implementationJournal, preparedEntry };
}
