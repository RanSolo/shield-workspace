...
test("harvest resolves portable paths and keeps unknown return explicit", async () => {
  ...
  await writeFile(registryPath, stableJson({
    schemaVersion: 1,
    tools: [{
      name: "helper",
      path: "./helper.mjs",
...
test("harvest calculates only observed numeric return", async () => {
  ...
  await writeFile(registryPath, stableJson({
    schemaVersion: 1,
    tools: [{
      name: "helper", path: toolPath, trigger: "Repeated check", purpose: "Automate it",
      inputs: [], outputs: [], minutesInvested: 10, minutesAvoidedPerUse: 4, reuseCount: 3,
      errorsPrevented: [], evidenceImproved: [], recommendation: "retain-local",
    }],
  }));
  const report = await harvestTools({ registryPath });
