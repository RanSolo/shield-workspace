...
4. Resolve config and state discovery independently:
   - Config precedence and effective directory: `GH_CONFIG_DIR` itself; otherwise `XDG_CONFIG_HOME/gh`; otherwise Windows `AppData/GitHub CLI`; otherwise the platform home fallback `.config/gh`.
   - State precedence and effective directory: `XDG_STATE_HOME/gh`; otherwise Windows `LocalAppData/GitHub CLI`; otherwise the platform home fallback `.local/state/gh`.
   - The platform home variable remains admitted whenever it is the state fallback, including when `GH_CONFIG_DIR` is present.
5. Derive those exact effective `gh` directories first, then require each effective config/state directory to be absolute and outside both source and mission roots after no-follow canonicalization of its nearest existing ancestor. Validation never substitutes the raw environment-variable parent for the effective directory. Relative, missing, symlink-ambiguous, equal, or nested effective directories block before spawning `gh` with stable redacted reasons `credential_environment_unsafe` or `credential_state_unavailable`. Rejected path values never appear in diagnostics.
...
- Effective-directory tests cover external parent values whose appended `gh`, `GitHub CLI`, `.config/gh`, or `.local/state/gh` directory becomes equal to or nested beneath either source or mission root.
