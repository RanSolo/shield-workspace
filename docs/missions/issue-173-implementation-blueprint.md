...
4. Obtain `packetBytes` from its descriptor `.value`; reject it if `types.isProxy(packetBytes)` is true or it is not a genuine `Uint8Array` with typed-array internal slots. Read its backing buffer through the intrinsic typed-array `buffer` getter, not an overridable ordinary property, and reject it as `malformed_input` if `node:util`'s `types.isSharedArrayBuffer` reports a `SharedArrayBuffer`. Then synchronously copy it into a new `Uint8Array` before the first `await`. All later packet validation, canonicalization, and hashing use only that copy.
...
4. Canonicalize without a trailing newline: emit `null`, `true`, and `false` literally; preserve array order; sort object keys lexicographically by UTF-16 code-unit order; encode strings with JSON escaping using lowercase hex for required `\\u00xx` control escapes and otherwise the shortest valid JSON representation. For each JSON number token, parse its grammar as an exact decimal mathematical value and also parse it to IEEE-754 binary64. Reject it as `malformed_packet` unless the binary64 result is finite and the input token's exact decimal value equals that binary64 value. Compute the ECMAScript shortest round-trippable decimal for that binary64 value and additionally reject unless the emitted decimal's exact mathematical value equals the same binary64 value. Emit that shortest decimal, with any accepted negative zero emitted as `0`. For every accepted numeric class, canonicalize -> parse -> canonicalize must reproduce byte-identical canonical numeric text; failure of that idempotence condition is `malformed_packet`.
...
- reject the full exact binary64-0.1 expansion `0.1000000000000000055511151231257827021181583404541015625`: its input decimal equals the binary64 value, but ECMAScript's shortest emitted `0.1` does not have that exact mathematical value;
...
- assert canonicalize -> parse -> canonicalize byte idempotence for accepted zero/negative-zero, safe and exactly representable integers, binary-exact fractions including `0.5`, exponent inputs, and exactly representable high-magnitude values.
...
- After marker write, lock-file sync, and lock-parent sync, but before any receipt-log read or mutation, reopen the lock directory entry with no-follow semantics. Require its `dev` and `ino` to match the retained acquisition values and its bytes to equal the exact canonical marker line. Disappearance, unlink, replacement, type drift, inode/device drift, marker drift, reopen/read failure, or ambiguity returns `recovery_required` with no receipt-log mutation or `executionDisposition`.
...
- Input snapshot tests reject an outer proxy without invoking its traps; reject class, array, null-prototype, inherited, symbol-keyed, extra-keyed, non-enumerable, and accessor-backed inputs; prove a `packetBytes` getter is never invoked; reject a proxied typed array and a `Uint8Array` backed by `SharedArrayBuffer`; and prove mutation after call entry cannot alter the synchronous byte snapshot.
- Numeric tests assert every required accept/reject vector above, including explicit rejection of `0.1`, the full exact binary64-0.1 expansion, unsafe integer rounding, overflow, underflow-to-zero, and high-precision rounding, plus byte-idempotent canonicalize -> parse -> canonicalize for every accepted numeric class.
...
- Post-sync acquisition tests inject lock-entry unlink and same-path replacement before the mandatory no-follow reopen; both return `recovery_required`, perform no receipt-log read/mutation, and omit `executionDisposition`. Additional drift tests cover changed canonical marker bytes and mismatched `dev`/`ino`.
